...
import math
from utils.gazebo_helpers import GazeboTile, GazeboDimensions
...
class ConcatImage:
    ...
    def process_column_image(self, dir_name, image_dir, tile_boundaries, temp_output_dir) -> list[str]:
        image_list = []
...
class HeightmapGenerator(ConcatImage):
    ...
    def get_amsl(self, lat: float, lon: float):
        ...
        dem_tile_path = os.path.join(globalParam.DEM_PATH, str(globalParam.DEM_RESOLUTION), str(tile_x),
                                     str(tile_y) + '.png')
    ...
    def generate_rgb_heightmap(self, model_path, boundaries, zoomlevel) -> str:
        ...
        height_map = ((cropped_image_float[:, :, 2] * 256 * 256 + cropped_image_float[:, :, 1] * 256 +
                       cropped_image_float[:, :, 0]) * 0.1) - 10000
        self.max_height = np.max(height_map)
        self.min_height = np.min(height_map)
        
        height_img_normalized = (
                (height_map - np.min(height_map)) / (np.max(height_map) - np.min(height_map)) * 255).astype(
            np.uint8)
        ...
        self.heightmap.save(os.path.join(globalParam.GAZEBO_MODEL_PATH, model, 'textures', model + '_height_map.tif'),
                            format="TIFF")
...
class OrthoGenerator(ConcatImage):
    ...
    def generate_ortho_tiles(self, path: str, zoomlevel, model_name, boundaries, world_dimensions: GazeboDimensions) -> list[GazeboTile]:
        """
        ...
        stitched_image_path = model_name + '_aerial.png'
        cv2.imwrite(os.path.join(globalParam.GAZEBO_MODEL_PATH, model_name, 'textures', stitched_image_path),
                    stitched_image, compression_params)
        
        return [
            GazeboTile(stitched_image_path).from_dimensions(world_dimensions)
        ]
...
class GazeboTerrianGenerator(HeightmapGenerator, OrthoGenerator):
    ...
    def gen_config(self) -> None:
        ...
        FileWriter.write_config_file(template, self.model_name,
    ...
    def gen_world(self) -> None:
        ...
        FileWriter.write_world_file(template, self.model_name, launch_cord["latitude"], launch_cord["longitude"],
                                    os.path.join(globalParam.GAZEBO_MODEL_PATH, self.model_name),
                                    launch_cord["altitude"], helipad_exist)
        FileWriter.write_world_file(template, self.model_name, launch_cord["latitude"], launch_cord["longitude"],
                                    globalParam.GAZEBO_WORLD_PATH, launch_cord["altitude"], helipad_exist)
    ...
    def get_world_dimensions(self) -> GazeboDimensions:
        """
        ...
        return GazeboDimensions(self.size_x, self.size_y, self.size_z, pose_x, pose_y, pose_z)
    ...
    def generate_gazebo_world(self):
        ...
        maptile_utiles.dir_check(os.path.join(globalParam.GAZEBO_MODEL_PATH, self.model_name, 'textures'),
        ...
        if os.path.isfile(os.path.join(self.tile_path, 'metadata.json')) and self.tile_path != '':
            ...
            w_dim = self.get_world_dimensions()
            
            # Height Tile take small size in Megabytes
            gazebo_height_tile = GazeboTile(height_image).from_dimensions(w_dim)
            gazebo_ortho_tiles = self.generate_ortho_tiles(self.tile_path, self.zoom_level, self.model_name, self.boundaries, w_dim)
            ...
            self.gen_world()
            print("Generate gazebo model files are save to : ",
                  os.path.join(globalParam.GAZEBO_MODEL_PATH, os.path.basename(self.tile_path)))
